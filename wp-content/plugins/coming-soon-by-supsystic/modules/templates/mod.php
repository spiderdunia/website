...
class templatesScs extends moduleScs {
    ...
    public function init() {
        ...
        frameScs::_()->addScript('scsAcPromoScript', SCS_JS_PATH. 'acPromoScript.js');
        frameScs::_()->addStyle('scsAcPromoStyle', SCS_CSS_PATH. 'acPromoStyle.css');
	...
	public function loadCoreJs() {
      ...
      $show = true;
      $acSubscribe = get_option('scs_ac_subscribe', false);
      if (!empty($acSubscribe)) {
        $show = false;
      }
      $acDisabled = get_option('scs_ac_disabled', false);
      if (!empty($acDisabled)) {
        $show = false;
      }
      $acRemind = get_option('scs_ac_remind', false);
      if (!empty($acRemind)) {
